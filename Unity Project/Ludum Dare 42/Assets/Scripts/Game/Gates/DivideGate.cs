...
public class DivideGate : Gate
{
	public DivideGate( CellCoordinates _coordinates, ObjectOrientation _orientation )
		: base( FindFootprint( _coordinates, _orientation ), FindInputs( _coordinates, _orientation ), FindOutputs( _coordinates, _orientation ), GateType.Divide )
	{
	}
	...
	public override void DoOperation( int[] _inputs )
	{
		if ( _inputs.Length != 2 )
		{
			Debug.LogError( "DivideGate: Expected 2 inputs got " + _inputs.Length );
		}
